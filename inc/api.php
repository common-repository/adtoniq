...
class AdtoniqAPI
{
  ...
  function adtoniq_api_render() {
    if ($this->saved === 'saved') {
      adtoniq_post_operation('apikey-saved');
  ...
  ?>
    <div class="well" data-help="apikey">
      ...
      <div class="well-container">
        ...
        <p class="lead">
          <?php if (strlen($this->_apiKey) === 0) { ?>
            You can get ad block analytics by <a id="signIntoAdtoniqCloudAcct" href="<?php echo esc_attr(ADTONIQ_ACCT_SERVER); ?>">signing up for a free Adtoniq Cloud account</a>.
          <?php } else { ?>
            Thank you for using Adtoniq Cloud. You can sign in to your Adtoniq Cloud account to
            authorize additional websites.
          <?php } ?>
        </p>
      </div>
